...
1. **高性能**：API 服务应具备低延迟和高吞吐量的能力，以适应高并发场景。
2. **低资源消耗**：在相同吞吐量和响应速度下，占用的系统资源（CPU、内存）越低越好。
3. **开发效率**：开发效率应高于大部分语言/框架，以减少开发成本和提升业务快速响应效率。这里强调一点，我认为的开发效率聚集的真实开发的效率，不包括学习成本。因为对于学习成本来说，不同人的学习能力是不一样的，而且若团队决定选用某一技术，那自然会寻找会这门技术的人员。
4. **可扩展性**：API 服务应具备良好的扩展性，以适应业务需求和用户需求。结合现在的技术生态，这一点对于大部分语言和框架来说都不太是问题了，实在不行前面再挂个反向代理或LSB服务也能做到服务节点可扩展。而扩展性要做的工作量，更多在后面数据存储和一致性上。
...
这里使用了 `Constructor` 宏来自动生成 `new` 构造函数（`注：Rust 中并没有构建函数的概念，但社区预定熟成使用 new 来构建对象。`），宏展开后生成的代码类似如下：
